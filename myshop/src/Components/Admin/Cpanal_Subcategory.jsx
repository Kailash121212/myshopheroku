...
export default function Cpanal_Subcategory() {
...
async function deleteData(_id){
  var rawdata= await fetch("/subcategory/"+_id,{
    method:"delete",
    headers:{
      authorization:localStorage.getItem("token"),
      username:localStorage.getItem("username")
  ...
  return (
    <>
       <div className='row mt-5'>
        ...
        <h5 className='background text-light text-center p-2'>Subcategory Section<Link to="/admin-add-Subcategory" className="btn text-light"><i className="fa fa-plus text-light "></i></Link></h5>
                ...
                <tbody>
                    ...
                    {subcategory.map((item,index)=>{
                      return <tr key={index}>
                          <td>{item._id}</td>
                          <td>{item.name}</td>
                          <td><Link to={"/admin-update-subcategory/"+item._id} className='btn text-primary' ><i className="material-icons">upgrade </i></Link></td>
